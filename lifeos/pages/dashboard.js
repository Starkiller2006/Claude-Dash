...
export default function Dashboard() {
  ...
  const promptIndex = useState(0)[0];
  ...
  // Fetch on mount, then auto-refresh every 15 minutes
  useEffect(() => {
    fetchWhoop();
    ...
    <div style={{ minHeight:"100vh", background:"#0a0a0f", fontFamily:"'DM Mono','Courier New',monospace", color:"#e8e4dc", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:24 }} className="fadein" key={section}>
          ...
          {section === "Business" && (
            <div>
              <div className="section-title">💼 Business HQ</div>
              <div className="grid2" style={{ marginBottom:16 }}>
              ...
              <div className="card" style={{ marginTop:16 }}>
                <div style={{ fontSize:12, color:"#555", marginBottom:12 }}>WEEKLY GOAL PROGRESS</div>
                {[{label:"Revenue Goal",pct:68,color:"#c8f264"},{label:"Client Outreach",pct:45,color:"#60a5fa"},{label:"Content Published",pct:80,color:"#a78bfa"},{label:"Team Tasks",pct:55,color:"#fbbf24"}].map(g=>(
                  <div key={g.label} style={{ marginBottom:14 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                      <span style={{ fontSize:13 }}>{g.label}</span><span style={{ fontSize:12, color:g.color }}>{g.pct}%</span>
                    </div>
          ...
          {section === "Fitness" && (
            <div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <div className="section-title" style={{ margin:0 }}>🏋️ Fitness — Live WHOOP Data</div>
              ...
              <div className="grid2" style={{ marginBottom:16 }}>
                <div className="card">
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>BODY METRICS (WHOOP)</div>
                  {[
                    { label:"Resting HR", value: restingHR ? `${restingHR} bpm` : null, color:"#f87171" },
                ...
                <div className="card">
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>RECENT WORKOUTS (WHOOP)</div>
                  {whoopLoading
          ...
          {section === "Sleep" && (
            <div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <div className="section-title" style={{ margin:0 }}>😴 Sleep — Live WHOOP Data</div>
                {!whoopLoading && !whoopError && <span className="whoop-badge">🟢 Live</span>}
              </div>
              ...
              {!whoopLoading && sleepTonight && (
                <div className="card" style={{ marginBottom:16 }}>
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>LAST NIGHT — SLEEP STAGES</div>
                  {[
              ...
              {/* 7-night sleep chart */}
              <div className="card">
                <div style={{ fontSize:12, color:"#555", marginBottom:16 }}>7-NIGHT SLEEP HISTORY</div>
                {whoopLoading
          ...
          {section === "Finance" && (
            <div>
              ...
              <div className="grid2">
                <div className="card">
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>MONTHLY BUDGET</div>
                  {[{cat:"Housing",spent:2200,budget:2200},{cat:"Food",spent:480,budget:600},{cat:"Transport",spent:220,budget:300},{cat:"Entertainment",spent:180,budget:200},{cat:"Business",spent:840,budget:1000}].map(b=>(
                    <div key={b.cat} style={{ marginBottom:12 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                        <span style={{ fontSize:13 }}>{b.cat}</span>
                        <span style={{ fontSize:12, color:b.spent>b.budget?"#f87171":"#4ade80" }}>${b.spent}/${b.budget}</span>
                      </div>
                      <div className="progress-bar"><div className="progress-fill" style={{ width:`${Math.min(b.spent/b.budget*100,100)}%`, background:b.spent>b.budget?"#f87171":"#4ade80" }}/></div>
                    </div>
                  ))}
                </div>
                <div className="card">
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>FINANCIAL GOALS</div>
                  {[{goal:"Emergency Fund",current:18000,target:25000,color:"#60a5fa"},{goal:"Investment Goal",current:89340,target:100000,color:"#a78bfa"},{goal:"Business Reserve",current:22780,target:30000,color:"#c8f264"},{goal:"Vacation Fund",current:3200,target:5000,color:"#fbbf24"}].map(g=>(
                    <div key={g.goal} style={{ marginBottom:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                        <span style={{ fontSize:13 }}>{g.goal}</span>
                        <span style={{ fontSize:12, color:g.color }}>{Math.round(g.current/g.target*100)}%</span>
                      </div>
                      <div className="progress-bar"><div className="progress-fill" style={{ width:`${g.current/g.target*100}%`, background:g.color }}/></div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {section === "Journal" && (
            <div>
              ...
              <div className="card" style={{ marginBottom:16 }}>
                ...
                <div style={{ fontSize:16, lineHeight:1.6, fontStyle:"italic", color:"#aaa" }}>"{journalPrompts[promptIndex]}"</div>
              </div>
              <div className="card" style={{ marginBottom:16 }}>
          ...
          {section === "Focus" && (
            <div>
              <div className="section-title">🎯 Focus Mode</div>
              ...
              <div className="grid2">
                <div className="card">
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>DEEP WORK LOG — TODAY</div>
                  {[{task:"Client proposal",duration:"2×25min",time:"9:00 AM"},{task:"Financial review",duration:"1×45min",time:"11:00 AM"},{task:"Content creation",duration:"2×25min",time:"2:00 PM"}].map((s,i)=>(
                    <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:"1px solid #1e1e2e" }}>
                      <div><div style={{ fontSize:13 }}>{s.task}</div><div style={{ fontSize:11, color:"#555" }}>{s.time}</div></div>
                      <span style={{ fontSize:12, color:"#c8f264" }}>{s.duration}</span>
                    </div>
                  ))}
                </div>
                <div className="card">
                  <div style={{ fontSize:12, color:"#555", marginBottom:14 }}>DISTRACTION BLOCKLIST</div>
                  {["Instagram","Twitter / X","YouTube","Reddit","News sites"].map((site,i)=>(
                    <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #1e1e2e" }}>
                      <span style={{ fontSize:13 }}>{site}</span>
                      <span style={{ fontSize:11, color:"#f87171", background:"#1d0a0a", padding:"2px 8px", borderRadius:4 }}>Blocked</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {section === "Habits" && (
            <div>
              <div className="section-title">🔥 Habit Tracker</div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:16 }}>
